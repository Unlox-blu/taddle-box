...
import { chatService, type ChatMessage } from "../api/chat.api";
import { postsService } from "../../posts/api/posts.api";
import { chatSocketClient } from "../../../infrastructure/websocket/chat-socket";
import { themedAlert } from "../../../design-system/components/ThemedAlert";
import StateBlock from "../../../shell/components/StateBlock";
import {
  useActiveStatus,
  activeStatusLabel,
} from "../../users/state/ActiveStatusProvider";
...
export default function ChatScreen() {
  ...
  // Staged chat params (other user / community identity) — consumed exactly
  // once on mount so re-renders keep the same conversation identity.
  ...
  const otherUserId = staged?.otherUserId;
  const otherUser = staged?.otherUser;
  const isCommunityChat = !!staged?.isCommunityChat;
  const communityName = staged?.communityName;
  const communityAvatar = staged?.communityAvatar;
  ...
  const openPost = useCallback(
    async (postId: string) => {
      try {
        ...
        const post = res?.data;
        if (post) {
          stageScreenParams("reel", { post, feedItems: [post], isSinglePost: true });
          router.push(`/post/${postId}`);
        }
      } catch {
        stageScreenParams("reel", { post: { id: postId } });
        router.push(`/post/${postId}`);
      }
    },
    [router],
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (isCommunityChat) {
              router.push(`/community/${conversationId}`);
            } else if (otherUser) {
              stageScreenParams("user", { user: otherUser });
              router.push(`/user/${otherUser.username || otherUser.id}`);
            }
          }}
        >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            router.push({ pathname: "/search", params } as never);
          }}
        >
          <Ionicons
            name="search-outline"
