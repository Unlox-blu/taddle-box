          ...
          <SectionHeader
            ...
            actions={[
              ...
              },
              {
                icon: "trophy-outline",
                onPress: () =>
                  router.push({ pathname: "/leaderboards", params: { initialTab: "Events" } } as never),
              },
            ]}
            pills={TABS.map((t) => ({
              key: t.key,
            ...
            {activeTab === "all" && featuredList.length > 0 && (
              <FlatList
                ...
                renderItem={({ item: featured }) => (
                  <TouchableOpacity key={featured.id} activeOpacity={0.9} style={[styles.featCard, { width: FEATURED_CARD_W, marginHorizontal: 0 }]} onPress={() => router.push(`/events/${featured?.id}` as never)}>
                    {featured.banner ? (
                  <ImageBackground
...
const EventCard = React.memo(function EventCard({
  ...
  return (
    <TouchableOpacity activeOpacity={0.8} style={styles.evCard} onPress={() => router.push(`/events/${e?.id}` as never)}>
