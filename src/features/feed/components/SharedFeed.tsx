...
import PostCard from "./content-cards/types/post-card/PostCard";
...
import { useActiveContentTracker } from "../../progression/hooks/useActiveContentTracker";
import { stageScreenParams } from "../../../shared/state/screen-params";
import { resolveContentId } from "../../../shared/utils/content.utils";
import FeedCard from "./content-cards/FeedCard";
import type { FeedCtx, ContentItem } from "./content-cards/content-card.types";
import { getContentType } from "./content-cards/content-card.types";
import { createRowStyles } from "../../../design-system/components/row.styles";
...
export default function SharedFeed({
    ...
    (post: Post) => {
      if (post.author) {
        if (currentUser?.id && post.author.id === currentUser.id) {
          ...
          router.push(`/user/${post.author.username || post.author.id}` as never);
        }
      }
    },
  ...
  const feedCtx: FeedCtx = useMemo(
    () => ({
      ...
      refresh: () => onRefresh?.(),
      openPost: (post) => {
        // Stage the tapped post + the visible feed so the reel viewer seeds
        // instantly and supports swipe-through; the URL carries only the id.
        stageScreenParams("reel", { post, feedItems: posts });
      ...
      openUser: (user) => {
        if (currentUser?.id && user?.id === currentUser.id) {
          ...
          router.push(`/user/${user?.username || user?.id}` as never);
        }
      },
      openCommunity: (slug) => router.push(`/community/${slug}` as never),
      openGames: (id?: string) => {
        if (id) {
      ...
      openSettings: () => router.push("/settings" as never),
      openNotifications: () => {},
      openFollowRequests: () => router.push("/follow-requests" as never),
      addHashtag: (tag) => router.push({ pathname: "/search", params: { query: tag } } as never),
      trackLayout,
      preloadPostId,
    ...
    [
      rowStyles, colors, router, isFocused, activeContentId,
