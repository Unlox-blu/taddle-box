...
import { highlightService, Highlight } from "../../community/api/highlights.api";
  ...
  const renderItem: ListRenderItem<Highlight> = ({ item, index }) => {
    ...
    const handlePress = (item: Highlight) => {
      ...
      if (item.type === "game") {
        ...
        router.navigate({ pathname: "/games", params: { openGameId: gameId, autoPlay: "true" } } as never);
      } else if (item.type === "event") {
        const eventId = item.sourceId || item.id.replace(/^event-/, '');
        router.push(`/events/${eventId}` as never);
      } else if (item.type === "community") {
        router.push(`/community/${item.sourceSlug || item.sourceId}` as never);
