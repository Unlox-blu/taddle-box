...
import { radii, fontSizes, spacing, type ColorPalette } from "../../../../../../design-system";
...
import PollBlock from "../../../../../posts/components/PollBlock";
...
import {
  ...
  makePostCardStyles,
  RollingText,
} from "./components/PostCardShared";
...
function PostCardInner({
            ...
            if (mentionMatch) {
              ...
              return (
                <Text
                  ...
                  onPress={() => router.push(`/user/${name}` as never)}
                >
                  @{name}
                </Text>
              );
            ...
            if (hashMatch) {
              return (
                <Text
                  ...
                  onPress={() =>
                    router.push({ pathname: "/search", params: { query: hashMatch[1], tab: "hashtags" } } as never)
                  }
                >
                  #{hashMatch[1]}
                </Text>
            ...
            if (communityMatch) {
              return (
                <Text
                  ...
                  onPress={() => router.push(`/community/${communityMatch[1]}` as never)}
                >
                  c/{communityMatch[1]}
                </Text>
              );
            ...
            if (plainCommunityMatch) {
              return (
                <Text
                  key={i}
                  style={{ color: colors.cyanLight, fontWeight: "700" }}
                  onPress={() => router.push(`/community/${plainCommunityMatch[1]}` as never)}
                >
            ...
            if (part.startsWith("@")) {
              return (
                <Text
                  key={i}
                  style={{ color: colors.primaryLight, fontWeight: "700" }}
                  onPress={() => router.push(`/user/${part.slice(1)}` as never)}
                >
                  {part}
                </Text>
              );
            ...
            if (part.startsWith("#")) {
              return (
                <Text
                  ...
                  style={{ color: colors.cyanLight }}
                  onPress={() =>
                    router.push({ pathname: "/search", params: { query: part.replace("#", ""), tab: "hashtags" } } as never)
                  }
                >
                  {part}
                </Text>
  ...
  const openPostThread = useCallback(
    async (target: Post) => {
      ...
      stageScreenParams("reel", { post: dest, isSinglePost: true });
      router.push(`/post/${dest?.id}` as never);
    },
    [router],
  );
