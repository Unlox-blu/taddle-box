...
import { colors, fontSizes, spacing, radii } from '../../../design-system';
...
export default function StoryViewerScreen() {
  ...
  // The story deck is staged (not URL-encodable); consume it exactly once on
  // mount. No sender stages a deck today — the route stays registered for
  // deep links but renders nothing until a deck is provided.
  const [deck] = useState(() => takeScreenParams<{ stories?: Story[]; initialIndex?: number }>('story'));
  const stories = deck?.stories ?? [];
  const initialIndex = Math.min(deck?.initialIndex ?? 0, Math.max(0, stories.length - 1));
  ...
  // No sender stages a story deck today — the route stays registered for
  // deep links but renders nothing until a deck is provided.
