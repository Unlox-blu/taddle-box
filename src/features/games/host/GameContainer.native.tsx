/**
 * GameHost — runtime-driven game router (app + web).
 *
 ...
 *   GameHost resolves: which component to render via RUNTIME_TO_COMPONENT
...
} from "../runtime/GameRuntimeRegistry";
...
/**
 * GameHost — the single entry point for rendering any game.
 *
 * Usage:
 *   <GameHost
 *     game={{
 ...
 *   />
 */
export default function GameContainer({ game, gameProps, resolvedAssets }: GameContainerProps) {
  const {
    slug,
    runtime,
    runtimeVersion = 1,
  ...
  // ── Web Runtime ──────────────────────────────────────────────────────
  if (runtimeType === "web") {
    const WebGameHost = React.lazy(() => import("./GameContainer.web"));

    if (!game.bundle?.url) {
      warn(`[GameHost] Web game ${runtime || slug} has no bundle URL`);
  ...
  if (!runtime || !isRuntimeSupported(runtime, runtimeVersion)) {
    warn(`[GameHost] Unsupported runtime: ${runtime}@${runtimeVersion}`);
  ...
  if (!isProtocolCompatible(runtime, runtimeVersion, backendProtocolVersion)) {
    warn(`[GameHost] Protocol mismatch: backend=${backendProtocolVersion}, runtime=${runtime}@${runtimeVersion}`);
  ...
  if (!isVersionCompatible(minAppVersion)) {
    warn(`[GameHost] App version mismatch: need ${minAppVersion}, have ${"APP_VERSION"}`);
