...
import type { HomeStackParamList, Post } from '../../../shared/types';
import { commentService, Comment } from '../api/comments.api';
  ...
  const renderParsedText = (text: string) => {
    ...
    return text.split(/(\{@\}\[[^\]]+\]\([^)]+\)|\{#\}\[[^\]]+\]\([^)]+\)|@\w+|#\w+)/g).map((part, i) => {
      ...
      if (mentionMatch) {
        ...
        return (
          <Text key={i} style={{ color: colors.primaryLight, fontWeight: '600' }} onPress={() => { handleClose(); router.push(`/user/${name}` as never); }}>
            @{name}
          </Text>
        );
      }
      if (part.startsWith('@')) {
        return (
          <Text key={i} style={{ color: colors.primaryLight, fontWeight: '600' }} onPress={() => { handleClose(); router.push(`/user/${part.slice(1)}` as never); }}>
            {part}
          </Text>
        );
      }
  ...
  const renderComment = (comment: Comment, isReply = false, rootComment?: Comment) => (
    <View key={comment.id} style={styles.commentWrapper}>
      <View style={styles.commentRow}>
        <TouchableOpacity onPress={() => { handleClose(); router.push(`/user/${comment.author?.username || comment.author?.id}` as never); }}>
