...
import { userService } from "../../users/api/users.api";
import { chatService } from "../../chat/api/chat.api";
import { useAuth } from "../../auth/state/AuthProvider";
import XPProgressBar from "../../progression/components/XPProgressBar";
import LevelInfoModal from "../../progression/components/LevelInfoModal";
import AchievementInfoModal from "../../progression/components/AchievementInfoModal";
...
import { activeStatusIndicator } from "../../users/state/ActiveStatusProvider";
import CommentsModal from "../../posts/components/CommentsModal";
import ActiveStatusDot from "../../users/components/ActiveStatusDot";
import { notificationService } from "../../notifications/api/notifications.api";
...
import type { XPUpdatedPayload } from "../../../shared/types";
import { themedAlert } from "../../../design-system/components/ThemedAlert";
import BioText, { normalizeUrl } from "../../users/components/BioText";
import BrandedLottieLoader from "../../../shell/components/BrandedLoader";
...
export default function SharedProfile({
  ...
  // Message button resolves (or creates) the DM conversation first — the chat
  // route expects a real conversation id (same flow as the chat inbox).
  const handleMessage = async () => {
    try {
      const conversationId = await chatService.getOrCreateConversation(
        user?.id,
      );
      stageScreenParams("chat", { otherUserId: user?.id, otherUser: user });
      router.push(`/chat/${conversationId}`);
    } catch (e: any) {
      themedAlert(
        "Cannot Message",
        e?.response?.data?.message || "You can only message mutual followers.",
      );
    }
  };
              ...
              : [
                  {
                    ...
                    emoji: "🎨",
                    color: "purple",
                    isUnlocked: postCount > 0 || isOwnProfile,
                  },
                  {
                    id: "b2",
                    name: "XP Pioneer",
      ...
      profileTab === "mentions" ? (
        ...
        <SharedFeed
          ...
          onLike={async (id) => {
            ...
            await postsService
              .toggleLike(id, !!row.data.isLiked)
              .catch(() => {});
          }}
          onSave={async (id) => {
            const row = posts.find((p) => p.id === id);
            ...
            setPosts((prev) =>
              prev.map((p) =>
                p.id !== id
                  ? p
                  : { ...p, data: { ...p.data, isSaved: !p.data.isSaved } },
              ),
            );
            await postsService
              .toggleSave(id, !!row.data.isSaved)
              .catch(() => {});
          }}
          ListHeaderComponent={profileHeader}
          ListEmptyComponent={
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      onClose();
                      router.push(
                        `/user/${item?.username || item?.id}` as never,
                      );
                    }}
                  >
