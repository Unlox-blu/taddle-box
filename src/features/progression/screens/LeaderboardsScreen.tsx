...
import type { LeaderboardsChangedPayload } from '../../../shared/types';
...
export default function LeaderboardsScreen() {
  ...
  const defaultTab = (initialTab?.toLowerCase() as LeaderboardType) || 'feed';
...
function LeaderboardRow({ entry }: { entry: WeeklyLeaderboardEntry }) {
  ...
  const handlePress = () => {
    if (entry.subtitle?.startsWith('@')) {
      ...
      stageScreenParams('user', { user: { username, name: entry.title, avatarUrl: entry.avatarUrl, handle: username } });
