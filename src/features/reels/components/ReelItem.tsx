...
import {
  ...
  formatInstagramTime,
  ZoomableMedia,
} from "../../feed/components/content-cards/types/post-card/components/PostCardShared";
          ...
          <FlatList
            ...
            renderItem={({ item: u }: any) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  onClose();
                  stageScreenParams("user", { user: u });
                  router.push(`/user/${u?.username || u?.id}`);
                }}
              >
                <View
                  style={{
...
export default React.memo(function ReelItem({
  ...
  const handleRepostPress = useCallback(
    (orig: Post) => {
      ...
      stageScreenParams("reel", { post: orig, feedItems: [orig], feedContext: "home" });
      router.push(`/post/${orig?.id}`);
    },
    [router],
  );
              ...
              {post.content ? (
                <>
                  <ParsedReelText
                    text={post.content}
                    onMentionPress={(name) => {
                      stageScreenParams("user", { user: { username: name } });
