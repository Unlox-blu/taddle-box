...
import { searchService, type SearchType } from "../api/search.api";
import { userService } from "../../users/api/users.api";
import { communityService } from "../../community/api/community.api";
import { hashtagService } from "../../posts/api/hashtags.api";
import { mapNotificationRow } from "../../notifications/api/notifications.api";
import { walletService } from "../../wallet/api/wallet.api";
import { xpService } from "../../progression/api/xp.api";
import { postsService } from "../../posts/api/posts.api";
import type { HomeStackParamList, Post, Transaction } from "../../../shared/types";
import StateBlock from "../../../shell/components/StateBlock";
import BrandedLoader from "../../../shell/components/BrandedLoader";
...
export default function SearchScreen() {
  ...
  const initialScope = params?.scopeCommunity || "";
  ...
  const initialAuthor = params?.authorFilter || "";
  ...
  const selectSuggestion = (item: any) => {
    if (suggestionKind === "user") {
      ...
      else if (item.id === "bookmarks") router.push("/bookmarks" as never);
      else if (item.id === "notifications") router.push("/notifications" as never);
      setSuggestionsVisible(false);
      setQuery("");
      return;
    }
  ...
  // events: time window). The fixed-order pills (people/community/games)
      ...
      >
        ...
        >
          ...
          {/* Sort/filter modal — hidden in the settings scope (its items have
              ...
              fixed-order pills (people/community/games) are showing, where
