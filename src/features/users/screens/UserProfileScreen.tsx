...
import SharedProfile from '../../profile/components/SharedProfile';
...
export default function UserProfileScreen() {
  ...
  // Staged user object — consumed exactly once on mount so re-renders keep
  // the same initial profile.
  const [staged] = useState(() =>
    takeScreenParams<{ user?: User | null; openPostId?: string; openPost?: Post }>('user'),
  );
  const initialUser = staged?.user ?? null;
  const openPostId = staged?.openPostId;
  const openPost = staged?.openPost;
