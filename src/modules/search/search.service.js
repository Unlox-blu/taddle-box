...
class SearchService {
  ...
  async search({
    ...
    try {
      switch (type) {
        case 'people': {
          if (!query) return await this.discoverPeople({ userId, page, limit, offset });
        ...
        case 'events': {
          if (!query && !filter && !bookmarked) return await this.discoverEvents({ limit, offset });
        ...
        case 'games': {
          if (!query) return await this.discoverGames({ limit, offset });
  ...
  // The individual searches already return their top/trending content, which the
