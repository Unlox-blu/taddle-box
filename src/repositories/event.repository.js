...
const create = async (data) => {
  const { rows } = await pool.query(
    ...
    [data.organizerId, data.communityId || null, data.title, data.description || null,
    data.coverImageUrl || null, data.eventType || 'online', data.startTime, data.endTime,
    data.timezone || 'Asia/Kolkata', JSON.stringify(data.location || {}),
    data.isFree !== false, data.ticketPriceCents || 0, data.currency || 'INR',
    data.maxAttendees || null, data.registrationDeadline || null, data.tags || []]
  );
  return rows[0];
};

const update = async (eventId, fields) => {
  const allowed = ['title', 'description', 'start_time', 'end_time', 'location', 'status', 'max_attendees', 'tags', 'is_featured'];
...
const softDelete = async (eventId) => {
  await pool.query(`UPDATE ${EventModel.TABLE} SET deleted_at = NOW(), status = 'cancelled' WHERE id = $1`, [eventId]);
};
...
const removeAttendee = async (eventId, userId) => {
  await pool.query(`DELETE FROM ${EventModel.ATTENDEES_TABLE} WHERE event_id = $1 AND user_id = $2`, [eventId, userId]);
};

const getAttendee = async (eventId, userId) => {
  const { rows } = await pool.query(
    `SELECT * FROM ${EventModel.ATTENDEES_TABLE} WHERE event_id = $1 AND user_id = $2`, [eventId, userId]
  );
  return rows[0] || null;
};
...
const incrementAttendeeCount = async (id) => pool.query(`UPDATE ${EventModel.TABLE} SET attendee_count = attendee_count + 1 WHERE id = $1`, [id]);
const decrementAttendeeCount = async (id) => pool.query(`UPDATE ${EventModel.TABLE} SET attendee_count = GREATEST(0, attendee_count - 1) WHERE id = $1`, [id]);
const updateAttendeeStatus = async (eventId, userId, status) =>
  pool.query(`UPDATE ${EventModel.ATTENDEES_TABLE} SET status = $1 WHERE event_id = $2 AND user_id = $3`, [status, eventId, userId]);
const confirmPayment = async (eventId, userId, rp) =>
  pool.query(`UPDATE ${EventModel.ATTENDEES_TABLE} SET razorpay_payment_id = $1, status = 'registered' WHERE event_id = $2 AND user_id = $3`, [rp.paymentId, eventId, userId]);

module.exports = {
  findById, browse, create, update, softDelete,
  addAttendee, removeAttendee, getAttendee, getAttendees,
  incrementAttendeeCount, decrementAttendeeCount, updateAttendeeStatus, confirmPayment,
};
