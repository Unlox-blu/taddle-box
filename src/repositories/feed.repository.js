...
const getUserPreferences = async (userId) => {
  const { rows } = await pool.query(
    `SELECT * FROM user_feed_preferences WHERE user_id = $1`, [userId]
  );
...
module.exports = { getPersonalizedPosts, recordInteraction, getUserPreferences, upsertUserPreferences };
