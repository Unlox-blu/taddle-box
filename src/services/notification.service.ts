...
// Maps ONE raw backend notification row into the app's Notification shape:
// relative time, fallback avatar, stored-type → mappedType routing, deep-link
// payloads and stacked-notification meta. Shared by getNotifications and the
// Search screen's notifications scope (which fetches through the unified
// search API and runs this same mapping over its rows).
export const mapNotificationRow = (n: any): Notification => {
  const now = new Date();
...
export const notificationService = {
  getNotifications: async (
    ...
    meta: {
      ...
      hasNext?: boolean;
      hasPrev?: boolean;
      // Server-computed type pills (All / Likes / Comments / Follows) with
      // per-bucket counts under the same q/time filters.
      types?: { type: string; label: string; count: number }[];
    };
  }> => {
    const typeParam = type ? `&type=${encodeURIComponent(type)}` : "";
    const unreadParam = unreadOnly ? "&unread=true" : "";
    // Server-side search — same params as global search: q is the free-text
    // term, time is the window (created_at cutoff), sort is accepted for
    // parity (notifications stay newest-first server-side).
    const qParam = q ? `&q=${encodeURIComponent(q)}` : "";
    const sortParam =
      sort && sort !== "relevance" ? `&sort=${encodeURIComponent(sort)}` : "";
    const timeParam =
      time && time !== "all_time" ? `&time=${encodeURIComponent(time)}` : "";
    const response = await apiClient.get(
      `/notifications?page=${page}&limit=${limit}${unreadParam}${typeParam}${qParam}${sortParam}${timeParam}`,
    );

    return {
      data: (response.data?.data || []).map(mapNotificationRow),
